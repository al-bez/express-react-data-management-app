...
const Councilors = () => {
  ...
  const handleNameInputValue = (event) => {
    ...
    setNameFilteredData([
      ...filter.filterByName(councilors, event.target.value),
    ]);
  };
    ...
    <Root>
      ...
      <AffairsWrapper>
        <OptionContainer>
          <InputWrapper>
            <TextField
              value={affairsInputValue}
              onChange={handleAffairsInput}
              type="date"
            />
          </InputWrapper>
        </OptionContainer>
        <div>Affairs</div>
        {affairsInputValue &&
          affairsFiltered.map((el) => {
            return <AffairItem key={uuid()} id={uuid()} date={el.updated} />;
          })}
        {!affairsInputValue &&
          affairs.map((el) => {
